...
dirtiff = ls(strcat(directory, '\*.tiff'));
...
mkdir(strcat(directory, '\Results'));
mkdir(strcat(directory, '\Results\Dispersion Coefficients'));
mkdir(strcat(directory, '\Results\Raw Data'));
mkdir(strcat(directory, '\Results\Raw Data\Roundness'));
mkdir(strcat(directory, '\Results\Raw Data\Distances'));
mkdir(strcat(directory, '\Results\Raw Data\Angles'));
mkdir(strcat(directory, '\Rejected'));
...
fin = fopen(strcat(directory, '\genotypes.qed'), 'rt');
...
if fin < 0
    ...
    fout = fopen(strcat(directory, '\genotypes.qed'), 'wt');
...
%% Centroids
    ...
    %% Add data to structure
        ...
        csvwrite(strcat(directory, '\Results\Raw Data\Roundness\', dirlist(i, :), '.csv'), qobject.ratio);
        csvwrite(strcat(directory, '\Results\Raw Data\Distances\', dirlist(i, :), '.csv'), qobject.distance');
        csvwrite(strcat(directory, '\Results\Raw Data\Angles\', dirlist(i, :), '.csv'), qobject.angle);
...
saveas(gcf, strcat(directory, '\Results\', geno{1}, '___', geno{2}, '.eps'), 'psc2');
saveas(gcf, strcat(directory, '\Results\', geno{1}, '___', geno{2}, '.tif'));
...
qresult(uratio, geno, strcat(directory, '\Results\Dispersion Coefficients\Roundness.csv'));
qresult(udist, geno, strcat(directory, '\Results\Dispersion Coefficients\Distances.csv'));
qresult(uangle, geno, strcat(directory, '\Results\Dispersion Coefficients\Angles.csv'));
